    ...
    const WorkExpBody = (
        <div className='w-full flex flex-col gap-2'>
            ...
            <div className='flex'>
                ...
                <InputControl label="Country" placeholder="E.g. India" value={values.cstate} onChange={(event) => setvalues((prev) => ({ ...prev, cstate: event.target.value }))}></InputControl>
            </div>
            <div className='flex'>
                <InputControl label="Start Date" type="month" value={values.wStart} onChange={(event) => setvalues((prev) => ({ ...prev, wStart: event.target.value }))}></InputControl>
        ...
        <div className='flex flex-col md:gap-7 items-center min-h-[540px] md:min-w-[80%] shadow-xl pt-1 pb-6 border border-slate-100'>
            ...
            <div className='flex flex-wrap gap-5'>
                {activeInformation?.details ? activeInformation?.details?.map((item, index) => (
                    <div key={item.id + index} className={`${activeChipIndex===index?"bg-blue-500":""}'cursor-pointer flex gap-3 items-center bg-gray-400 px-3 rounded-3xl justify-center'`} onClick={()=>{setactiveChipIndex(index)}}>
